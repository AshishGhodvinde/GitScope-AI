...
import type { Repository, FileEntity, SummaryResponse, ChatHistoryEntry } from '../types';
...
export function WorkspaceLayout() {
  ...
  const [fileData,    setFileData]    = useState<FileEntity[] | null>(null);
  ...
  // Poll status if INDEXING
  useEffect(() => {
    if (!repo || repo.status !== 'INDEXING') return;
    const interval = setInterval(async () => {
      try {
        const data = await getRepository(repoId);
        if (data.status !== 'INDEXING') { setRepo(data); clearInterval(interval); }
      } catch (err) {
  ...
  if (loadingRepo) {
    return (
      <div
        className="relative z-10 flex h-[calc(100vh-64px)] w-screen items-center justify-center"
  ...
  const statusBadge = {
    INDEXED:  <span className="badge badge-green text-[9px]">Indexed</span>,
    INDEXING: <span className="badge badge-yellow text-[9px] animate-pulse">Indexing…</span>,
    FAILED:   <span className="badge badge-red text-[9px]">Failed</span>,
  }[repo.status];
  ...
  return (
    <motion.div
      ...
      className="relative z-10 flex h-[calc(100vh-64px)] w-screen overflow-hidden font-sans"
      style={{ background: 'var(--canvas-bg)', color: 'var(--text-primary)' }}
    >

      {/* ── COLUMN 1: Operational Sidebar (20% width) ── */}
      <div
        className="w-1/5 min-w-[240px] max-w-[320px] shrink-0 flex flex-col h-full overflow-hidden select-none border-r"
        style={{ background: 'var(--glass-sidebar)', borderColor: 'var(--divider)' }}
      >
        {/* Home portal */}
        <div
        ...
        >
          ...
          <div
            ...
            style={{
              ...
              borderColor: 'rgba(15,191,62,0.20)',
              color: 'var(--accent)',
            }}
          >
        ...
        <div
          ...
          style={{ background: 'var(--canvas-bg)', borderColor: 'var(--divider)' }}
        >
          <div className="flex items-start gap-3 mb-3">
            <div
              className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0 border"
              style={{ background: 'rgba(15,191,62,0.08)', borderColor: 'rgba(15,191,62,0.25)' }}
            >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs truncate mb-1" style={{ color: 'var(--text-secondary)' }}>{repo.owner}</p>
              <a
                href={repo.url}
                target="_blank"
              ...
              history.map(entry => (
                <button
                  ...
                  onMouseEnter={e => {
                    ...
                    (e.currentTarget as HTMLButtonElement).style.background = 'rgba(15,191,62,0.04)';
                  }}
                  onMouseLeave={e => {
                    (e.currentTarget as HTMLButtonElement).style.borderColor = 'var(--divider)';
                    (e.currentTarget as HTMLButtonElement).style.background = 'var(--canvas-bg)';
                  }}
                >
                  <p
      ...
      {/* ── COLUMN 2: Center Main View (flex-1, min-w-0) ── */}
      <div className="flex-1 min-w-0 flex flex-col h-full overflow-hidden" style={{ background: 'var(--canvas-bg)' }}>
        <Outlet context={{
          repo,
          fileData,
          summary,
      ...
      <div
        className="w-1/4 min-w-[260px] max-w-[380px] shrink-0 flex flex-col h-full overflow-hidden select-none border-l"
        style={{ background: 'var(--glass-sidebar)', borderColor: 'var(--divider)' }}
      >
        <div
          ...
          style={{ borderBottom: '1px solid var(--divider)' }}
        >
          <Compass className="w-3.5 h-3.5" style={{ color: 'var(--text-muted)' }} />
          <span
            className="text-[10px] font-semibold uppercase tracking-widest"
            style={{ color: 'var(--text-secondary)' }}
          >
        ...
        <div className="flex-1 overflow-hidden p-3 min-h-0">
          {loadingFiles ? (
            ...
            <div className="h-full overflow-y-auto scrollbar-thin">
              <FileExplorer files={fileData.map(f => f.path)} />
            </div>
          ) : (
            <div className="flex flex-col items-center justify-center h-full py-8 text-center">
              <p className="text-xs font-mono" style={{ color: 'var(--text-muted)' }}>No files scanned.</p>
            </div>
          )}
        </div>
      </div>
